...
def clean_extra(lines):
    ...
    for ele in lines:
        ...
        if(ele ==','):
            ...
            if(ele == '}'):
                ans+=lines[writing:place]
                ans+=lines[place+1:index+1]
            else:
                ans+=lines[writing:index+1]
            writing = index+1
            start=0
    ans+=lines[writing:]
    return ans
...
def parse_string(code, debug=0, lineno=1, prefix='++'):
    ...
    if(verbose_flag):
        print("Lexing Done")
        print("Parsing Start")
    code1 = clean_extra(code)
    if(code1 != code):
        print("ERROR")
        exit(0)
    out1 = parser.parse(prefix + code1, lexer=lexer, debug=0)
    out2 = parser.parse(prefix + code, lexer=lexer, debug=0)
    if(out1!=out2):
        print("ERROR")
        exit(0)
    return out1
